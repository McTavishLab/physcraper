...
class AlignTreeTax(object):
    ...
    def edit_dict_key(self):
        ...
        for item in replace_key:
            ...
            item_edit = item_edit.replace("/", "_")
            self.otu_dict[item_edit] = self.otu_dict.pop(item)
            # debug(self.otu_dict.keys())
            #note: it's been automatically removed?
            #del self.otu_dict[item]
        for tax_lab in self.aln.taxon_namespace:
            tax_lab.label = tax_lab.label.replace("-", "_")
            tax_lab.label = tax_lab.label.replace("'", "_")
            tax_lab.label = tax_lab.label.replace("/", "_")

        for tax_lab in self.tre.taxon_namespace:
            tax_lab.label = tax_lab.label.replace("-", "_")
            tax_lab.label = tax_lab.label.replace("'", "_")
            tax_lab.label = tax_lab.label.replace("/", "_")
    ...
    def add_otu(self, gi, ids_obj, email=False):
        ...
        #### this gives me an open raise exception error, back to my implementation for now
        # try:
        ...
        ## this should probably be  in its own class or so.
        ## We are trying to access gi tax information from ATT and Physcraper class.
        # commented code here is replaced with the ids_obj - get_rank_info func
        ...
        try:
            ...
            try:
                ...
                ott = "OTT_{}".format(self.ps_otu)
                self.ps_otu += 1
        otu_id = "{}_{}".format(spn.replace(" ", "_"), gi)
        self.otu_dict[otu_id] = {}
        self.otu_dict[otu_id]['^ncbi:gi'] = gi
        self.otu_dict[otu_id]['^ncbi:accession'] = self.gi_dict[gi]['accession']
        self.otu_dict[otu_id]['^ncbi:title'] = self.gi_dict[gi]['title']
    ...
    def write_files(self, treepath="physcraper.tre", treeschema="newick", alnpath="physcraper.fas", alnschema="fasta"):
        ...
        self.aln.write(path="{}/{}".format(self.workdir, alnpath),
                       schema=alnschema)
        # hack to remove illegal characters in files
        self.rewrite_files(inputfn="physcraper.tre")
    ...
    def rewrite_files(self, inputfn):
        """rewrited the written files to remove illegal characters.
        TODO:rename otu-lables respectively
        """
        debug("rewrite files")
        import fileinput
        files = '{}/{}'.format(self.workdir, inputfn)
        # Does a list of files, and
        # redirects STDOUT to the file in question
        print(files)
        for line in fileinput.input(files, inplace=1):
            print(line.replace("'", "_").rstrip())
    ...
    def write_labelled(self, label, treepath="labelled.tre", alnpath="labelled.fas"):
        ...
        for taxon in tmp_tre.taxon_namespace:
            ...
            if self.otu_dict[taxon.label].get('^ot:ottTaxonName') != None:
                ...
                new_label = self.otu_dict[taxon.label].get("^ncbi:taxon")
            if new_label:
                if new_label in new_names:
                    new_label = "_".join([new_label, taxon.label])
            elif self.otu_dict[taxon.label].get("user:TaxonName"):
                new_label = self.otu_dict[taxon.label].get("user:TaxonName").replace(' ', '_')
                if new_label in new_names:
                    new_label = "_".join([new_label, taxon.label])
                ...
                new_label = self.otu_dict[taxon.label].get("^ot:originalLabel").replace(' ', '_')
                if new_label in new_names:
                    new_label = "_".join([new_label, taxon.label])
            elif self.otu_dict[taxon.label].get("^ncbi:taxon"):
                new_label = "_".join(["ncbi", str(self.otu_dict[taxon.label].get("^ncbi:taxon"))])
                if new_label in new_names:
                    new_label = "_".join([new_label, taxon.label])
...
class IdDicts(object):
    ...
    def map_gi_ncbi(self, gi):
        ...
        ## changed it to make use of the new get_rank_info function
            tax_name = self.get_rank_info(gi_id=gi)
            tax_id = self.otu_rank[tax_name]["taxon id"]
            # debug("try entrez.fetch")
            # Entrez.email = self.config.email
            # handle = Entrez.efetch(db="nucleotide", id=gi, retmode="xml")
            # read_handle = Entrez.read(handle)[0]
            # tax_name = read_handle['GBSeq_feature-table'][0]['GBFeature_quals'][0]['GBQualifier_value']
    ...
    def dump(self, filename=None):
        ...
        if filename:
            ...
            ofi = open("{}/id_pickle.p".format(self.workdir), "wb")
        pickle.dump(self, ofi)
...
#            blast_dir = self.blast_subdir
                        ...
                        for alignment in blast_record.alignments:
                            for hsp in alignment.hsps:
                                if float(hsp.expect) < float(self.config.e_value_thresh):
                                    ...
                                    if gi_list_mrca:
                                        if gi_id in gi_list_mrca:
                                            if gi_id not in self.data.gi_dict: #skip ones we already have (does it matter if these were delted? No...)
    ...
    def dump(self, filename=None):
        ...
        pickle.dump(self, ofi)        #TODO... write as proper nexml?!
    ...
    def align_query_seqs(self, papara_runname="extended"):
        ...
        try:
            ...
            # Something else went wrong while trying to run `wget`
                raise
        os.chdir(cwd)
        debug("{}/papara_alignment.{}".format(self.workdir, papara_runname))
    ...
    def est_full_tree(self):
        ...
        subprocess.call(["raxmlHPC", "-m", "GTRCAT",
                         ...
                         "-p", "1",
                         "-n", "{}".format(self.date)])
        os.chdir(cwd)#TODO mordir not always one down!
    ...
    def generate_streamed_alignment(self, treshold=None):
        ...
        if len(self.new_seqs) > 0:
            ...
            sys.stderr.write("NOOOOOOOO") #???? Miscommunication??: I'm talking about remove_identical_seqs
