...
class AlignTreeTax(object):
    ...
    def add_otu(self, gi, ids_obj, localblast=False):
...
class IdDicts(object):
    ...
    def get_rank_info(self, gi_id=False, taxon_name=False):
        ...
        if gi_id:
            ...
            #     sys.stderr.write("Cannot get sp info from gi: {}. I still have to find a solution for it".format(gi_id))
            #     debug("Cannot get sp info from gi: {}. I still have to find a solution for it".format(gi_id))
            ...
            if self.blacklist is not None and gi in self.blacklist:
                ...
                if len(seq.replace("-", "").replace("N", "")) > seq_len_cutoff:
                    if self.config.blast_loc == 'local':
                        ...
                        localblast = False
                    otu_id = self.data.add_otu(gi, self.ids, localblast)
                            ...
                            for item in self.sp_d[taxon_id]:
                                if '^ncbi:gi' in item:
                                    if self.config.blast_loc == 'local':
                                        ...
                                        localblast = False
                                    self.data.add_otu(item['^ncbi:gi'], self.ids, localblast)
