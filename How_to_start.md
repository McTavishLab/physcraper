...
#### 2.b) install a virtual environment
  This is very useful if you want to run it on a cluster and/or do not want to change already installed packages on your computer.
  A virtual environment will locally install the packages needed.

  `pip install virtualenv` 
  `virtualenv -p python2 NameOfYourENV`  # currently it only runs on python 2.7, you may need to just say `python` instead of `python2`, depending on your system

  To use the virtualmaschine you need to activate it before doing anything else. This needs to be done before you start installing software in your virtual maschine or before running physcraper.
